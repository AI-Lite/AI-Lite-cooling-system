...
Lr_1 = 0.06 # the step_size(learning rate) of graph1
    ...
    if (TIMES % 18) >= 15 or (TIMES % 18) == 0:
        ...
        if (len(STATE_INPUT) < 4) and (TIMES % 18 == 0): # 传感器失灵，此轮不训练，则将相应变量清空
            ...
            if TIMES > 288 * collect_smaple_date:
                ...
                # BASELINE = int(np.mean(ALL_PUE)*10*Baselinefactor*1.07)/10
                # pue_len = len(ALL_PUE)
                # PUE_sort = sorted(ALL_PUE)
                # len_pue = len(PUE_sort)
                # #BASELINE = int(np.mean(ALL_PUE) * 100 * Baselinefactor) / 100
                # BASELINE = (PUE_sort[int(len_pue * pue_fraction)] * 100 * Baselinefactor) / 100
                BASELINE = int(np.mean(ALL_PUE) * 100) / 100
                # BASELINE = np.mean(ALL_PUE)
                # PUE_latest = ALL_PUE[pue_len - 24:pue_len]
                # BASELINE = int(np.mean(ALL_PUE) * 10 * Baselinefactor) / 10
                #BASELINE = 1.3
                print("Baseline:", BASELINE)
                temp_penalty = weight_temp*math.log(1+math.exp(abs(cold_temp_mean-Temp_max)))
                rew_1 = -(pue_av + LOSS_AC_NUM - BASELINE)
                # if pue_av + LOSS_AC_NUM > BASELINE:
                #     rew_1 = -math.pow((pue_av + LOSS_AC_NUM - BASELINE),2)
                # # rew_1 = - math.pow((pue_av + LOSS_AC_NUM - BASELINE), 1)
                # else:
                #     rew_1 = math.pow((pue_av + LOSS_AC_NUM - BASELINE),2)
                if (cold_temp_mean >= Temp_max):
                    rew_1 -= 1.5*temp_penalty
                if  (cold_temp_mean <= Temp_min):
                    rew_1 -= weight_temp*math.log(1+math.exp(abs(cold_temp_mean-Temp_min)))
                STATE_INPUT, PUE_LIST, COLD_TEMP_MEAN = [], [], []  # 清空相应变量
                #rew_1  =  -(pue_av  + LOSS_AC_NUM - BASELINE)
                #rew_1 = -(math.exp(pue_av + LOSS_AC_NUM) - math.exp(Baseline))
                print('pue:', pue_av)
                LOSS_AC_NUM = 0
                if len(EP_OBS_1) != 0:
                    #DATA_Y.append(cold_temp_mean)
                ...
                if TIMES <= 288*train_data:
                    # ALL_PUE.append(jEnvState['InstantPue'])
                ...
                if TIMES >= 288*train_data:
                   ...
                   len_pue = len(PUE_sort)
                   BASELINE = int(np.mean(ALL_PUE) * 100) / 100
                   # BASELINE = int(PUE_sort[int(len_pue * pue_fraction)] * 100) / 100
                   label_1 = ac_action - 2
                   # BASELINE = 1.27
                   print("Baseline:", BASELINE)
                   # rew_1 = BASELINE - pue_av
                   if pue_av > BASELINE:
                        rew_1 = -math.pow(pue_av - BASELINE, 2)*8
                        # rew_1 = -math.pow(pue_av - BASELINE,1) * 3
                   else:
                        rew_1 = math.pow(pue_av - BASELINE, 2)*8
                        # rew_1 = -math.pow(pue_av - BASELINE, 1) * 2
                   if len(EP_OBS_1) != 0:
                       # DATA_Y.append(cold_temp_mean)
                       EP_REW_1.append(rew_1)
                       # model = model.fit(train_x,train_y)
                       ...
                       for i in range(Num_epi):  # 训练并存储模型
                           if ((int((TIMES / 18) / (Num_epi * Len_epi))) % 4 == 0):  # 训练频率错频
                               if np.abs(cold_temp_mean - TARGET_TEMP) <= epsilon:
                                   ...
                                   G1.sess1.run(G1.train_op_1,
                                                feed_dict={G1.observation_1: np.vstack(
                                                    EP_OBS_1[(i * Len_epi):((i + 1) * Len_epi)]),
                                                    G1.reward_1: reward_list_1[i],
                                                    G1.action_1: np.array(
                                                        EP_ACT_1[(i * Len_epi):((i + 1) * Len_epi)]),
                                                    G1.lrmult_1: Lrmult})
                               # G1.saver_1.save(G1.sess1, os.path.join(os.path.dirname(__file__), \
                               #                                  "model_pg/graph_1_ckpt/model.ckpt"), global_step=int(TIMES / 18))
                       EP_OBS_1, EP_OBS_2, EP_ACT_1, EP_ACT_2, EP_REW_1, EP_REW_2 = [], [], [], [], [], []  # 训练后清空当前存储的模型
