...
def acg_control_predict(dEnvState,G1):
    ...
    global train_PPO_num, old_input, old_action, old_reward,train_PPO_second
    ...
    if (TIMES % 18) >= 15 or (TIMES % 18) == 0:
        ...
        if (len(STATE_INPUT) < 4) and (TIMES % 18 == 0): # 传感器失灵，此轮不训练，则将相应变量清空
            ...
            if TIMES > 288 * collect_smaple_date:
                ...
                ac_action = np.random.choice(AC_CHOOSE, p=ac_select.ravel())
                # print("afdsddsdf",ac_action)
                label_1 = AC_CHOOSE.index(ac_action)
                print(ac_select)
                if (cold_temp_mean <= TARGET_TEMP - violation) and (label_1 >= int(Max_ac_num/2 + 1)-Ac_num_lower):
                    # label_1 = int(Max_ac_num/2 + 1)-Ac_num_lower
                    # ac_action = AC_CHOOSE[label_1]
                    # print(ac_select[0])
                    # print(Max_ac_num/2 + 1-Ac_num_lower)
                    ac_select[0][int(Max_ac_num/2 + 1)-Ac_num_lower:] = 0
                    if sum(ac_select[0]) > 0:
                        ac_select = ac_select/sum(ac_select[0])
                    else:
                        ac_select[0][:int(Max_ac_num/2 + 1)-Ac_num_lower] = [1/(int(Max_ac_num/2 + 1)-Ac_num_lower)
                                                                           for i in range(0,int(Max_ac_num/2 + 1)-Ac_num_lower)]
                    ac_action = np.random.choice(AC_CHOOSE, p=ac_select.ravel())
                    label_1 = AC_CHOOSE.index(ac_action)
                ...
                # label_1 = AC_CHOOSE.index(ac_action)
                AC_cold_temp = get_rack_cold_temp(jEnvState,Area)
                if (train_PPO_second > 1) and (old_action < AC_CHOOSE.index(sum(AC_LIST))):
                    change_penalty = 0.5
                else:
                    change_penalty = 0
                # old_action = AC_CHOOSE.index(sum(AC_LIST))
                ac_l = AC_LIST.copy()
                ...
                temp_penalty = weight_temp*math.log(1+math.exp(abs(cold_temp_mean-TARGET_LOW)))
                rew_1 = pue_av + LOSS_AC_NUM
                if (cold_temp_mean <= TARGET_LOW - temp_epsilon_upper):
                    rew_1 += temp_penalty
                if (cold_temp_mean >= TARGET_TEMP):
                # # if (cold_temp_mean <= TARGET_TEMP - temp_epsilon_upper):
                    rew_1 += weight_temp*math.log(1+math.exp(abs(cold_temp_mean-TARGET_TEMP))) # reward
                rew_1 = np.mean(ALL_PUE) - rew_1 - change_penalty
                print('pue:', pue_av)
                LOSS_AC_NUM = 0

                if rew_1 >= 0:
                    rew_1 = 10*rew_1*rew_1
                else:
                    rew_1 = -10*rew_1*rew_1

                if train_PPO_second > 1:
                ...
                if TIMES >= 288 * train_data:
                    ...
                    rew_1 = np.mean(ALL_PUE) - pue_av
                    if rew_1 >= 0:
                        rew_1 = 10 * rew_1 * rew_1
                    else:
                        rew_1 = -10 * rew_1 * rew_1
                    if np.abs(cold_temp_mean - TARGET_TEMP) <= epsilon:
                        train_PPO_num += 1
                    if (np.abs(cold_temp_mean - TARGET_TEMP) <= epsilon) & (train_PPO_num > 1):
                        G1.perceive(old_input, old_action, rew_1, input_1)
                    old_input = input_1.copy()
