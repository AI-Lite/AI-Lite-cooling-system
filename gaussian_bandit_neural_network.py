...
from neural_nework_for_ablation import TwoLayerNeuralNetwork
import time
import pandas
    ...
    if (TIMES % 18) >= 15 or (TIMES % 18) == 0:
        ...
        if (len(STATE_INPUT) < 4) and (TIMES % 18 == 0): # 传感器失灵，此轮不训练，则将相应变量清空
            ...
            if TIMES > 288 * collect_sample_date:
                ...
                nn_two_layer = TwoLayerNeuralNetwork()
                nn_two_layer.fit(Train_x, Train_y)
                pre_y = nn_two_layer.predict(train_x)
