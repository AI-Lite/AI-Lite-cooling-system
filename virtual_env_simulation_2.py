...
class EnvTemp(object):
    def __init__(self):
        ...
        # it_load = [[55, 88], [53, 88, 114, 139, 114, 88], [26, 53, 88, 114, 139, 114, 88, 53]]
        # it_load = [[88, 88], [88, 88, 88, 88, 88, 88], [88, 88, 88, 88, 88, 88, 88]]
        it_load = [[55, 88], [53, 88, 114, 139, 114, 88], [53, 53, 88, 114, 139, 114, 88, 53]]
        generator_load = it_load[2]
    ...
    def get(self):
        ...
        if self.__cur_idx != 288 - 1:
            ...
            self.__load_idx = (self.__load_idx + 1) % 8
        else:
            self.__cur_idx = 0
            self.num = self.num + 1
            # self.__load_idx = np.random.choice(list(range(len(self.__load_generatores))))
...
def get_hot_temp_mean__(_EnvState):
    fRackHotTempes = []
    for label in _EnvState:
        if label.find("RackHotTemp_") == 0:
            fRackHotTempes.append(_EnvState[label])
    return round(np.mean(fRackHotTempes), 1)
...
def get_env_state__():
    ...
    _EnvState["HotTempMean"] = get_hot_temp_mean__(_EnvState)
...
def main3():
    for num in range(2000):
        ...
        ac_on_nums = random.randint(2, 6)  # 至少开 2 台空调
        setting_temp_array = [random.randint(15, 22)] * 6
        setting_onoff_array = [0] * 6

        while (sum(setting_onoff_array) != ac_on_nums):
            random_index = random.randint(0, 5)
            if setting_onoff_array[random_index] == 0:
                setting_onoff_array[random_index] = 1

        set_setting_temp(setting_temp_array.copy())
        set_onoff(setting_onoff_array.copy())
...
if "__main__" == __name__:
    ...
    # main2()
    main3()
    # cold_temp_pd = pd.DataFrame(data=state_data)
    # cold_temp_pd.to_csv('state_data_2000.csv', encoding='utf-8', index=False, header=state_data[0].keys())
    # print('完成')