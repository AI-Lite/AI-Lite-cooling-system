    ...
    if (TIMES % 18) >= 15 or (TIMES % 18) == 0:
        ...
        if (len(STATE_INPUT) < 4) and (TIMES % 18 == 0): # 传感器失灵，此轮不训练，则将相应变量清空
            ...
            if TIMES > 288 * collect_sample_date:
                ...
                other_params = {'max_depth': 5, 'learning_rate': 0.20, 'silent': False}   # first_stage_use XGBoost regression  
                model_first_stage = MultiOutputRegressor(xgb.XGBRegressor(**other_params)).fit(history_config, history_performance)  # 
                pre_y_first_stage = model_first_stage.predict(candidate_config) # 
                label_1 = temp_decision(pre_y_first_stage, TARGET_TEMP) # 
