...
# Input_dim_1 = int(encode_len*3) + encode_len * (4)
Input_dim_1 = encode_len * 3 + encode_len * 5
#Input_dim_1 = 88
Input_dim_2 = Input_dim_1 + Max_ac_num   # the dimension of the input of the graph1
# Num_hidden_1 = 8  # the number of dimension of graph1
Num_hidden_1 = 8  # the number of dimension of graph1
...
action_taken = np.zeros((Max_ac_num-Min_ac_num+1))
current_time = 1
A_ininital = np.zeros((Max_ac_num-Min_ac_num+1,Input_dim_1,Input_dim_1))
for k in range(0,Max_ac_num-Min_ac_num+1):
    A_ininital[k] = np.eye(Input_dim_1)
B_reward = np.zeros((Max_ac_num-Min_ac_num+1,Input_dim_1))
alpha = 0.4
# alpha = 0.2
...
# weight_temp=0.05
weight_temp=0.1
temp_epsilon_upper = 4
temp_epsilon_lower = 3.5
pue_fraction = 0.25
...
def acg_control_predict(dEnvState,G1):
    ...
    global EP_OBS_1, EP_ACT_1,EP_REW_1,LOSS_AC_NUM,DATA_X,COLD_TEMP_MEAN,BASELINE
    global train_num, old_input, old_action, old_reward, A_ininital, B_reward, old_inputs, action_taken, current_time
    ...
    if (TIMES % 18) >= 15 or (TIMES % 18) == 0:
        ...
        if (len(STATE_INPUT) < 4) and (TIMES % 18 == 0): # 传感器失灵，此轮不训练，则将相应变量清空
            ...
            inputs = state_mean
            if train_num >= 1:
                input_1 = vec_embedding_1(old_inputs, AC_CHOOSE[old_action], encode_len)
            ...
            if TIMES > 288 * collect_sample_date:
                ...
                input_transpose = old_input.reshape((-1, 1))
                A_ininital[old_action] = A_ininital[old_action] + input_transpose * old_input
                B_reward[old_action] = B_reward[old_action] + 1 / (pue_av - 1.1) * old_input
                ...
                for action_label in action_candidate:
                    # print("bgin$$$$$",A_ininital[action_label])
                    # print(B_reward[action_label].reshape((-1, 1)))
                    A_matrix = np.linalg.inv(A_ininital[action_label])
                    # print(A_matrix)
                    theta_action = np.dot(A_matrix, B_reward[action_label].reshape((-1,1)))
                    # print(theta_action)
                    action_input = vec_embedding_1(inputs,AC_CHOOSE[action_label],encode_len)
                    action_input_transpose = action_input.reshape((-1,1))
                    # print(action_input)
                    # print(theta_action)
                    # math.pow(action_taken[action_label]+2, 0.6)
                    action_score = np.dot(action_input, theta_action) + alpha / math.pow(action_taken[action_label]+2, 0.7) \
                                   * math.sqrt(np.dot(action_input, np.dot(A_matrix, action_input_transpose)))
                    all_action_score.append(action_score)
                label_1 = np.argmax(all_action_score)
                label_1 = action_candidate[label_1]
                action_taken[label_1] = action_taken[label_1] + 1
                ...
                if (train_num > 1) and (abs(cold_temp_mean - TARGET_TEMP) <= 3):
                    old_input = input_1.copy()
                    input_transpose = old_input.reshape((-1, 1))
                    A_ininital[old_action] = A_ininital[old_action] + input_transpose * old_input
                    B_reward[old_action] = B_reward[old_action] + 1 / (pue_av - 1.1) * old_input
...
if "__main__" == __name__:
     G1 = PPO(Input_dim_1, Num_hidden_1, Out_dim_1)  # generate graph of setting_onoff
     times = 288 * 30
     collect_smaple = 1
     ratio_temp = []
     control_trace = []
